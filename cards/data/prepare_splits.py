...
Note: cards_val.jsonl and cards_test.jsonl use labels from `mapping/final_claims_dict.json`
(the post-review label set used by the published Nature paper) when an override is available
for a given text. Stratification is performed on the seed labels first to keep the val/test
partition byte-stable across runs.
...
def load_final_claims_overrides():
    """Load the revised label set used by the published Nature paper.

    `final_claims_dict.json` is keyed by raw congress_test text and stores
    the post-review label set (numpy-array repr, space-separated). It differs
    from the seed `true_claims` column on ~5% of items; metrics in the paper
    were computed against this revised set, so we promote it as canonical.
    Falls back to the seed labels for any text not present in the override.
    """
    path = os.path.join(DATA_DIR, 'mapping', 'final_claims_dict.json')
    if not os.path.exists(path):
        print(f"  WARNING: {path} not found — using seed labels only")
        return {}
    with open(path) as f:
        d = json.load(f)
    out = {}
    for text, raw in d.items():
        if not isinstance(raw, str):
            continue
        # numpy-array repr: "['6_1_3' '4_2_2' '4_1_5']"
        try:
...
def prepare_eval_splits():
    ...
    # Seed labels from the CSV (preserve original stratification)
    ...
    # Stratified 30/70 split — uses SEED labels so the partition is stable
    # across runs (model results in data/results/test/*.jsonl join by text).
    df_val, df_test = train_test_split(
        df, test_size=0.7, random_state=RANDOM_STATE, stratify=df['strat_key'],
    )

    # Promote the Nature paper's revised labels (final_claims_dict) over the
    # seed labels at write time. Partition is unchanged.
    ...
    for name, d in [('cards_val', df_val), ('cards_test', df_test)]:
        ...
        print(f"  {name}.jsonl: {len(records)} samples "
