# /// script
# requires-python = ">=3.10"
# dependencies = [
#     "llmcompressor @ git+https://github.com/vllm-project/llm-compressor.git@main",
#     "transformers>=5.2.0",
#     "tokenizers>=0.21",
#     "torch",
#     "huggingface_hub[hf_transfer]",
#     "accelerate",
# ]
#
# [tool.uv]
# override-dependencies = [
#     "transformers>=5.2.0",
# ]
# ///

"""
CARDS-Wind joint Qwen3.6-27B FP8 quantization (post-training, no retraining).

Source: iRanadheer/CARDS-Wind-Qwen3.6-27B (or C3DS/CARDS-Wind-Qwen3.6-27B)
Output: C3DS/CARDS-Wind-Qwen3.6-27B-FP8 (public)

FP8_DYNAMIC: per-channel FP8 weights (static), per-token FP8 activations (dynamic).
No calibration data needed. lm_head stays in bf16.

Usage:
  uv run quantize_cards_wind_fp8.py            # quantize and push public
  uv run quantize_cards_wind_fp8.py --no-push  # local only
"""

import argparse
import gc
import os
import shutil
import sys
import time

sys.stdout.reconfigure(line_buffering=True)
sys.stderr.reconfigure(line_buffering=True)

SRC  = os.environ.get("SRC_MODEL", "iRanadheer/CARDS-Wind-Qwen3.6-27B")
DST_ORG = os.environ.get("DST_ORG", "C3DS")
OUT  = f"{SRC.split('/')[-1]}-FP8"   # e.g. CARDS-Wind-Qwen3.6-27B-FP8 or Windy-Qwen3.5-27B-FP8
REPO = f"{DST_ORG}/{OUT}"

parser = argparse.ArgumentParser()
parser.add_argument("--no-push", dest="push", action="store_false", default=True)
parser.add_argument("--keep-local", action="store_true")
args = parser.parse_args()

os.environ.setdefault("HF_HUB_ENABLE_HF_TRANSFER", "1")

from huggingface_hub import login, HfApi
token = os.environ.get("HF_TOKEN")
if token:
    login(token=token)

import torch
from transformers import AutoModelForCausalLM, AutoTokenizer
from llmcompressor import oneshot
from llmcompressor.modifiers.quantization import QuantizationModifier

print(f"{'='*60}")
print(f"  Source:  {SRC}")
print(f"  Output:  ./{OUT}")
print(f"  Repo:    {REPO}  (public)")
print(f"  Push:    {args.push}")
print(f"  GPU:     {torch.cuda.get_device_name(0) if torch.cuda.is_available() else 'CPU'}")
if torch.cuda.is_available():
    print(f"  VRAM:    {torch.cuda.get_device_properties(0).total_memory / 1e9:.1f} GB")
print(f"{'='*60}\n")


print(f"[1/3] Loading {SRC} ...")
t = time.time()
tokenizer = AutoTokenizer.from_pretrained(SRC)
model = AutoModelForCausalLM.from_pretrained(
    SRC,
    dtype=torch.bfloat16,
    device_map="auto",
)
print(f"  loaded in {time.time() - t:.1f}s")

print(f"\n[2/3] Quantizing (FP8_DYNAMIC) ...")
t = time.time()
recipe = QuantizationModifier(
    targets="Linear",
    scheme="FP8_DYNAMIC",
    ignore=["lm_head"],
)
oneshot(model=model, recipe=recipe)
print(f"  quantized in {time.time() - t:.1f}s")

print(f"\n[3/3] Saving to ./{OUT} ...")
t = time.time()
model.save_pretrained(OUT, save_compressed=True)
tokenizer.save_pretrained(OUT)
print(f"  saved in {time.time() - t:.1f}s")

if args.push:
    print(f"\nPushing public -> {REPO} ...")
    t = time.time()
    api = HfApi()
    api.create_repo(REPO, private=False, exist_ok=True)
    api.upload_folder(
        folder_path=OUT,
        repo_id=REPO,
        repo_type="model",
        commit_message=f"Upload FP8 quantized {SRC.split('/')[-1]}",
    )
    print(f"  pushed in {time.time() - t:.1f}s")
    print(f"  https://huggingface.co/{REPO}")

del model, tokenizer
gc.collect()
if torch.cuda.is_available():
    torch.cuda.empty_cache()
    torch.cuda.synchronize()

if args.push and not args.keep_local:
    shutil.rmtree(OUT, ignore_errors=True)
    print(f"  cleaned local ./{OUT}")

print(f"\nDone. Serve with:")
print(f"  vllm serve {REPO} --max-model-len 4096 --trust-remote-code --language-model-only")
