"""
Train Qwen3.6-27B with Unsloth on single H200 GPU.

Usage (on RunPod pod):
    pip install unsloth datasets "trl>=0.12.0" "huggingface_hub[hf_transfer]" "transformers>=5.2.0" flash-linear-attention tensorboard
    huggingface-cli login
    tmux new -d -s train "python train_27b.py 2>&1 | tee train.log"
    tmux attach -t train
"""
...
MODEL_REPO = f"{HF_USERNAME}/CARDS-Qwen3.6-27B"
BASE_MODEL = "Qwen/Qwen3.6-27B"
MAX_SEQ_LENGTH = 4096
OUTPUT_DIR = "CARDS-Qwen3.6-27B"
...
try:
    ...
    api.upload_folder(folder_path=OUTPUT_DIR, repo_id=MODEL_REPO, repo_type="model",
                      commit_message="Upload 27B model")
    print(f"\nModel saved: https://huggingface.co/{MODEL_REPO}")
