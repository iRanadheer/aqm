...
- 27 val-only annotation corrections live in `annotation_patches.json`
  (audited in `annotation_errors.md`).
- `synthetic_zeros.jsonl` supplies C_0_0 negatives during the split
  stage (generated once via `generate_synthetic.py --mode zeros`).
...
### 4.2 Hyperparameters (matching 27B reference from cards-2pO-paper)
...
- `openrouter` — Anthropic prompt caching on system prompt
  ...
  ~10× cheaper after warmup, verified in `scripts/check_caching.py`).
...
- **`--mode augment`** (new) — weak-code augmentation for
  frames AND claims with teacher-labeling quality gate. Output
  schema matches `train_labels.jsonl`.
  - `--negatives` flag: inverts the quality gate. For FP-heavy codes,
    generates near-miss texts and keeps only rows where the teacher
    does NOT predict the target code. Teacher's labels become gold.

### 9.2 Pipeline

```
generate_synthetic.py --mode augment [--negatives]
    │
...
were recovered via a standalone `ft/merge_push_27b.py` script.
...
- [x] Annotation patches audited in `annotation_errors.md` with
      itemId-level notes.
- [x] Prompt engineering iterations tracked via codebook diffs in git
      history and prose in `prompt_optimization_approach.md`.
